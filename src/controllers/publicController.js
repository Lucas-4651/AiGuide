...
const [ais, types] = await Promise.all([AI.all(filters), AI.types()]);
res.render('pages/explore', { title: 'Explorer les IAs', ais, types, filters });
} catch (e) { next(e); }
};
...
const [tips, categories] = await Promise.all([Tip.all(cat), Tip.categories()]);
res.render('pages/tips', { title: 'Astuces IA', tips, categories, activeCategory: cat || null });
} catch (e) { next(e); }
};
...
res.render('pages/flashcards', { title: 'Flash Cards IA', cards, categories, activeCategory: cat || null });
} catch (e) { next(e); }
};
...
res.render('pages/roadmap', { title: 'Roadmap IA', roadmaps });
} catch (e) { next(e); }
};
...
if (!roadmap) { req.flash('error', 'Roadmap introuvable'); return res.redirect('/roadmap'); }
const completed = req.user ? await Progress.get(req.user.id, roadmap.id) : [];
res.render('pages/roadmap-detail', { title: roadmap.title, roadmap, completed });
} catch (e) { next(e); }
};
...
res.render('pages/compare', { title: 'Comparer les IAs', selected, allAIs });
} catch (e) { next(e); }
};
...
const result = await recommend(description.trim(), { type, pricing, difficulty });
res.render('pages/recommendations', { title: 'IAs recommandees', result, description });
} catch (e) { next(e); }
};
...
if (steps.length >= 1) await Badge.award(req.user.id, 'first_roadmap');
res.json({ ok:true });
} catch(e){ next(e); }
};