// ============================================================
//  src/models/index.js
//  Toutes les requetes DB centralisees ici
//  Pas d'ORM lourd — juste Knex + fonctions simples
// ============================================================
const db = require('../config/database');

// Utilitaire : parse JSON stocke en TEXT (SQLite)
...
// Hydrate un objet ai_tool (parse les champs JSON)
...
// Verifie si l'user peut encore generer un prompt aujourd'hui
...
// Incremente le compteur journalier de prompts
...
// Compte les prompts generes par un IP aujourd'hui (pour les invites)
guestCount: async (ip) => {
const today = new Date().toISOString().slice(0, 10);
const r = await db('prompts')
.where({ guest_ip: ip })
...
// ─────────────────────────────────────────────────────────
const Log = {
create : (data)  => db('activity_logs').insert(data),
recent : (limit) => db('activity_logs').orderBy('created_at', 'desc').limit(limit || 50),
forUser: (userId) => db('activity_logs').where({ user_id: userId }).orderBy('created_at', 'desc').limit(50),
stats  : ()      => db('activity_logs').select('action').count('id as count').groupBy('action').orderBy('count', 'desc'),
count  : ()      => db('activity_logs').count('id as c').first().then((r) => Number(r.c))
};
...
forUser:  (userId) => db('favorites').join('ai_tools','favorites.ai_id','ai_tools.id').where({ 'favorites.user_id': userId, 'ai_tools.is_active': true }).select('ai_tools.*','favorites.created_at as fav_at').orderBy('favorites.created_at','desc'),
...
let ch = await db('daily_challenges').where({ date, is_active: true }).first();
if (!ch) {
// Auto-génère un défi depuis flashcards ou tips
const card = await db('flash_cards').where({ is_active: true }).orderByRaw('RANDOM()').first();
if (card) {
const [id] = await db('daily_challenges').insert({
date, type: 'flashcard', ref_id: card.id,
challenge_text: 'Explique avec tes propres mots : "' + card.term + '"',
is_active: true, created_at: new Date().toISOString()
});
ch = await db('daily_challenges').where({ id }).first();
}
}
return ch;
},
...
'first_roadmap'    : { label:'En Route',         icon:'🗺️', desc:'Première étape roadmap cochée' },
